...
@MainActor
public final class WorkspaceCaptureFixture {
    private var unexpectedAlert: String?
    ...
    public func run() async throws {
        ...
        guard let outputPath = env["WORKSPACE_CAPTURE_ROOT"] else {
            throw captureError("Native capture is opt-in through the Actions workflow.")
        }
        ...
        let runnerTemp = URL(fileURLWithPath: try unwrap(env["RUNNER_TEMP"])).resolvingSymlinksInPath()
        ...
        let sourceSHA = try unwrap(env["CAPTURE_SOURCE_SHA"])
        var report = Report(
            sourceSHA: sourceSHA,
        ...
        do {
            ...
            let terminalProcess = try unwrap(terminal.process)
            ...
            let rootView = try unwrap(window.contentView)
            ...
            let transcript = try unwrap(model.activeTranscriptController)
            let encoder = JSONEncoder()
            ...
            let otherTranscript = try unwrap(model.activeTranscriptController)
            ...
            let otherOpener = try unwrap(findControl("show-session-details", in: rootView))
            ...
            let otherCloser = try unwrap(findControl("hide-session-details", in: rootView))
            ...
            let closer = try unwrap(findControl("hide-session-details", in: rootView))
            ...
            for (name, appearance, width, height, projectsVisible): (String, NSAppearance.Name, Int, Int, Bool) in [
                ...
                if !projectsVisible {
                    let rootView = try unwrap(window.contentView)
                    let projects = try unwrap(findView(NSTableView.self, in: rootView))
                    try require(window.makeFirstResponder(projects), "The project browser did not accept focus.")
                }
                navigation.showsProjects = projectsVisible
                window.appearance = try unwrap(NSAppearance(named: appearance))
                ...
                for attempt in 1...3 {
                    ...
                    window.contentView?.layoutSubtreeIfNeeded()
                    window.displayIfNeeded()
                    let container = try unwrap(findView(
                        TerminalsContainerView.self, in: try unwrap(window.contentView)
                    ))
                    ...
                    let detailsFrame = try unwrap(detailsButton.accessibilityFrame?())
                    ...
                    if !projectsVisible {
                        let previousWidth = try unwrap(compactTerminalWidth)
                    ...
                    let file = "\(name).png"
                    let png = try unwrap(NSBitmapImageRep(cgImage: image)
            ...
            let imageRef = try unwrap(terminal.kittyImageCapture.retainedImageMetadata().first)
            ...
            let imageControl = try unwrap(findControl(imageIdentifier, in: rootView))
            ...
            let commandWindow = try unwrap(window.attachedSheet)
            ...
            try await waitFor("The pinned preview lost its image after retained bytes were deleted.") {
                guard let sheet = window.attachedSheet else { return false }
                return self.findControl("transcript-image-preview", in: sheet, role: nil) != nil
            }
            model.selectSession(projectId: project.id, sessionId: sessions[1].id)
            ...
            let restoredRef = try unwrap(terminal.kittyImageCapture.retainedImageMetadata().first)
            ...
            let openBeforeDrawerClose = try unwrap(findControl(restoredIdentifier, in: rootView))
            ...
            try await Task.sleep(for: .milliseconds(500))
            window.contentView?.layoutSubtreeIfNeeded()
            let projects = try unwrap(findView(NSTableView.self, in: try unwrap(window.contentView)))
            try require(window.makeFirstResponder(projects), "The empty project browser did not accept focus.")
            navigation.showsProjects = false
            try await Task.sleep(for: .milliseconds(500))
            window.contentView?.layoutSubtreeIfNeeded()
    ...
    private func transcriptFixtureImage() throws -> Data {
        ...
        let destination = try unwrap(CGImageDestinationCreateWithData(
            data, UTType.png.identifier as CFString, 1, nil
        ))
        CGImageDestinationAddImage(destination, try unwrap(context.makeImage()), nil)
    ...
    private func captureTranscriptImage(window: NSWindow, file: String, output: URL) async throws -> TranscriptImageProof {
        ...
        let shared = try unwrap(content.windows.first { $0.windowID == CGWindowID(captureWindow.windowNumber) })
        ...
        let png = try unwrap(NSBitmapImageRep(cgImage: image).representation(using: .png, properties: [:]))
        try png.write(to: output.appendingPathComponent("images/\(file)"), options: .atomic)
        let request = VNRecognizeTextRequest()
        request.recognitionLevel = .accurate
        request.recognitionLanguages = ["en-US"]
    ...
    private func require(_ condition: Bool, _ message: String) throws {
        if let unexpectedAlert { throw captureError("The fixture attempted an alert: \(unexpectedAlert)") }
        if !condition { throw captureError(message) }
    }

    private func unwrap<T>(_ value: T?, file: StaticString = #fileID, line: UInt = #line) throws -> T {
        guard let value else { throw captureError("Missing required fixture value at \(file):\(line).") }
        return value
    }
}
#endif
